...
def cosine_sim(i, j):
    ...
    if len(uij) == 0:
        return 0
    dot_prod = m[uij, i].T.dot(m[uij, j]).data[0]
    # dot_prod = 0 if len(dot_prod) == 0 else dot_prod[0]
    norm_i = np.linalg.norm(m[uij, i].data)
    if norm_i == 0:
        return 0
    norm_j = np.linalg.norm(m[uij, j].data)
    if norm_j == 0:
        return 0
...
if __name__ == "__main__":
    ...
    if not PRECALCULATION:
        ...
        if SAVE_RESULTS:
            with open('../result/cf_item_{}_{}_{}'.format(N, DIST_FUNC,
